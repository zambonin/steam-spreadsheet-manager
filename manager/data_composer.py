...
def parse_games_data(api_key, steamid):
    ...
    """
    return {
        str(game.pop("appid")): game for game in get_games(api_key, steamid)
    }
...
def parse_prices_data(app_list, itad_api_key, itad_region, itad_country):
    ...
    """
    ids, current, lowest = get_prices(
        app_list, itad_api_key, itad_region, itad_country
    )

    prices = {}
    for key, value in ids["data"].items():
        app = key.split("/")[1]
        prices[app] = {"orig": 0.0, "lowest": 0.0, "discount": 0.0}
        try:
            price = current["data"][value]["list"]
            prices[app]["orig"] = price[0]["price_old"] if price else 0.0
            prices[app]["lowest"] = lowest["data"][value]["price"]
            prices[app]["discount"] = lowest["data"][value]["cut"] / 100
        except KeyError:
            continue
...
def shape(
    ...
    prices = parse_prices_data(
        list(games), itad_api_key, itad_region, itad_country
    )
